...
/**
 * 空白签章，仅在签章区显示一条签名，在pdf表层不叠加任何印章，锁定PDF，禁止增加批注等附加信息。适用于保全文档使用。
 *
 */
public class SignByLockDemo {

	private static final Logger log = LoggerFactory
			.getLogger(SignByLockDemo.class);
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 生成签章外观，空白签章，仅在签章区显示一条签名，在pdf表层不叠加任何印章，锁定PDF，禁止增加批注等附加信息。适用于保全文档使用。
			PdfSignatureAppearance sap = SignAppearanceGenerator
					.signLock(pdfReader, serialNo);
			...
			PdfReader.savePdfFile(signedPdfBytes, "d:/lock.signed.pdf");
			log.info("签署完毕，文档已保存到" + "d:/" + "lock.signed.pdf");
