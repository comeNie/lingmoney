...
/**
 * 
 * 定位pdf中的全部指定关键字，在关键字坐标中心处自动叠加盖章，支持图片类型的签章
 *
 */
public class SignByAllKeywordDemo {

	private static final Logger log = LoggerFactory
			.getLogger(SignByAllKeywordDemo.class);
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (KeywordLocaton keywordPos : KeywordLocator
					.getAllLocations(pdfBytes, "人民币")) {
				// 签章的序列号，最长18位，该序列号在同一PDF中必须唯一，建议与数据库保持映射关系，以便日后可查该签章相关信息
				String serialNo = "demo:" + System.currentTimeMillis();

				// 读取图片文件到字节流
				byte[] imageBytes = ImageUtil.getBytes("stamp.png");
				...
				PdfSignatureAppearance sap = SignAppearanceGenerator.signImage(
						pdfBytes, imageBytes, serialNo, keywordPos, 0.1875f,
						null);
			...
			PdfReader.savePdfFile(pdfBytes, "d:/allkeyword.signed.pdf");
			log.info("签署完毕，文档已保存到" + "d:/" + "allkeyword.signed.pdf");
