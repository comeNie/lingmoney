...
/**
 * 
 * 使用图片作为印章进行签章，如果不需要使用背景，背景字段传入null。适用于一般图片印章签章。
 *
 */
public class SignByImageDemo {

	private static final Logger log = LoggerFactory
			.getLogger(SignByImageDemo.class);
	...
	public static void main(String[] args) {
		try {
			...
			PdfReader pdfReader = new PdfReader("D:/jkxy.pdf");
...
//					ImageBackground.QCODE_SIZE_SMALL, 0, 16);
			...
			// 读取图片文件到字节流
			byte[] imageBytes = ImageUtil.getBytes("D:/20180116101503.png");
			...
			PdfSignatureAppearance sap = SignAppearanceGenerator.signImage(
					pdfReader, imageBytes, serialNo, signOnPage, 1f, 1f,
					0.12f, null);
			...
			// 第三方平台业务订单号
			String orderId = "A1113e2523ecer5f1111";
			...
			// 签署人手机号(个人签署时必须传递)
			receiver.put("phone", "15520325321");
			// 签署人证件类型 0=居民身份证,1=护照,8=企业营业执照
			receiver.put("cardType", "0");
			// 签署人证件号
			receiver.put("cardNumber", "110123199811021011");
			// 签署人真实姓名
			receiver.put("realName", "张素素");
			// 签署人邮箱（企业签署时必须传递）
			...
			PdfReader.savePdfFile(signedPdfBytes, "d:/image.signed.pdf");
			log.info("签署完毕，文档已保存到" + "d:/" + "image.signed.pdf");
