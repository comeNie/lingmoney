...
function publish() {
	...
	showLoading();
	$.getJSON(
			"/rest/gift/lingbaoActivityBanner/publish?r=" + Math.random() + "&id=" + rows[0].id,
			function(info) {
				if (info.code == 200) {
...
function del(){
	...
	$.messager.confirm('确认','您确认想要删除记录吗？',function(r){    
	    if (r){
			$.getJSON("/rest/gift/lingbaoActivityBanner/delete?r=" + Math.random() + "&id=" + rows[0].id,
			    function(info) {
			       if(info.code==200){
...
function saveAndUpdate(){
	showLoading();
	$('#ffAdd').form('submit',{
		url: '/rest/gift/lingbaoActivityBanner/saveAndUpdate',
		ajax:true,
		success:function(data){
