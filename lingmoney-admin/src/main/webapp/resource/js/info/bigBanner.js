...
function publish() {
	...
	showLoading();
	$.getJSON("/rest/base/bigBanner/publish?r=" + Math.random() + "&id=" + rows[0].id,
			function(info) {
				if (info.code == 200) {
...
function refresh() {
	$.getJSON("/rest/base/bigBanner/refresh",
	function(info) {
		console.log(info);
...
function reload(){
	showLoading();
	$.getJSON("/rest/base/bigBanner/reload?r=" + Math.random(),
	    function(info) {
	       if(info.code==200){
	    	   $("#adGrid").datagrid('reload');
	       }else{
...
function del(){
	...
	$.messager.confirm('确认','您确认想要删除记录吗？',function(r){    
	    if (r){
			$.getJSON("/rest/base/bigBanner/delete?r=" + Math.random() + "&id=" + rows[0].id,
			    function(info) {
			       if(info.code==200){
...
function save(){
	showLoading();
	$('#ffAdd').form('submit',{
		url: '/rest/base/bigBanner/upload',
		ajax:true,
...
function update(){
	showLoading();
	$('#ffEdit').form('submit',{
		url: '/rest/base/bigBanner/update',
		ajax:true,
		success:function(data){
			var json = JSON.parse(data);
			if(json.code == 200){
