...
function exportExcel2(){
	...
	//动态设置表单的action
	$('#searchFrm').attr('action','/rest/product/giftExchangeInfo/exportExcel').submit();     
...
function formatType(value, row) {
	if (value == 0) {
		return "<font color='red'>50元话费充值卡</font>";
	} else if (value == 1) {
...
function delivery(){
	...
	if(rows[0].type==0){// 50元话费
		$('#DivDelivery').css("height","290px");
		$(".recharge").show();
		$("#label").text("充值卡类型：");
		$('#giftName').combobox({
		    url:'/resource/json/rechargeType.json',
		    editable:false,
		    required:true,
		    valueField:'name',
		    textField:'name'
		...
		$("#label").text("超值大礼包：");
		$('#giftName').combobox({
		    url:'/resource/json/superGift.json',
		    editable:false,
		    required:true,
		    valueField:'name',
		    textField:'name'
...
function processingDelivery(){
	...
	$('#ffDelivery').form('submit',{
		url: '/rest/product/giftExchangeInfo/processingDelivery?r=' + Math.random(),
		ajax:true,
		success:function(data){
...
function receipt(){
    ...
    for(var i=0;i<rows.length;i++){  
    	if(rows[i].status == 0){
    		$.messager.alert("提示", "部分礼品未发货", "error");
    		return
    	}
    	if(rows[i].status == 2){
    		$.messager.alert("提示", "部分礼品已收货", "error");
    		return
    	}
    	ids += rows[i].id;  
        if(i < rows.length-1){  
	...
	$.messager.confirm('确认','确认收货？',function(r){    
	    if (r){
			$.getJSON("/rest/product/giftExchangeInfo/comfirmReceipt?r=" + Math.random() + "&ids=" + ids,
			    function(info) {
			       if(info.code==200){
