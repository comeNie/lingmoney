	 ...
	 $.messager.confirm("操作提示", "您确定要更改任务状态吗？", function (data) { 
         if (data) {  
         	...
         	if(status != null && status != ""){
         		$.ajax({
         	        type: "post",
         	        url: '/rest/schedule/changeJobStatus',
         	        data: {
         	        	jobId:id,
         	        	status:status
         	            },
...
function editFun(id) {
    ...
    parent.$.modalDialog({
        ...
        width : 550,
        height : 300,
        href : '/rest/schedule/scheduleEdit?jobId=' + id + '',
        buttons : [ {
            text : '确定',
            handler : function() {
...
function addFun() {
	parent.$.modalDialog({
		...
		width : 600,
		height : 350,
		href : '/rest/schedule/scheduleAdd',
		buttons : [ {
			text : '添加',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
function executeNow() {
	...
	$.messager.confirm('提示', '确定要执行吗?', function(flag) {
		if(flag) {
			$.ajax({
				url:'/rest/schedule/executeJobNow',
...
function editConfirm() {
	$.ajax({
		url : "/rest/schedule/editSchedule",
		type : "post",
...
function removeSchedule() {
	...
	if (row != null) {
		$.messager.confirm('提示', '确定要删除吗?', function(flag) {
			if(flag) {
				$.ajax({
					url : '/rest/schedule/removeSchedule',
					type : 'post',
					data : {"jobId" : row.jobId},
					success : function (data) {
						if (data.code == 200) {
