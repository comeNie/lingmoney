...
public class TransferDebtServiceImpl implements TransferDebtService {
	...
	private final String postUrl = PropertiesUtil
			.getPropertiesByKey("BANK_POST_URL");
...
//			}
	...
	@Override
	public PageInfo transferDebtAssignment(String uId, int clientType,
			String appId, String amount, String remark, String pCode)
			throws Exception {
		...
		// 预计剩余收益
		String preincome = String.valueOf(Integer.parseInt(amount) / ResultNumber.TWO.getNumber());

		String returnUrl = "https://222.249.236.5:8443/bank/transferDebtCallBack"
				+ "/" + uId;
		Map<String, String> map = transferDebt.transferDebtAssignment(
				clientType, appId, "2", oldreqseqno, oldreqnumber, oldreqname,
				hxAccount.getAcNo(), hxAccount.getAcName(), amount, preincome,
				remark, returnUrl);
	...
	@Override
	public PageInfo transferDebtAssignmentSearch(String appId,
			String oldreqseqno) throws Exception {
		PageInfo pageInfo = new PageInfo();
		JSONObject json = new JSONObject();
		pageInfo.setObj(json);

		Document result = transferDebt.transferDebtAssignmentSearch(appId,
				oldreqseqno);
		...
		json.put("oldreqseqno",
				result.selectSingleNode("//OLDREQSEQNO").getText());
