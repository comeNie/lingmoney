...
import com.mrbt.lingmoney.wap.vo.user.UserAddressVo;
import com.mrbt.lingmoney.wap.vo.user.UsersBaseInfoVo;
...
public class UserAccountSetController {
    ...
    public Object queryUserAccountSetInfo(HttpServletRequest request) {
        ...
        PageInfo pageInfo = new PageInfo();
        try {
            if (StringUtils.isNotBlank(CommonMethodUtil.getUidBySession(request))) {
                pageInfo = usersService.queryUsersInfo(CommonMethodUtil.getRedisUidKeyByRequest(request));
