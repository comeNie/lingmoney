...
public class SmsVerifyCode {
    ...
    public @ResponseBody Object sendModiPhoneVerifyOld(HttpServletResponse response, HttpServletRequest request,
		...
		PageInfo pageInfo = new PageInfo();
		try {
            String key = CommonMethodUtil.getRedisUidKeyByRequest(request);
            pageInfo = smsService.sendModiPhoneVerifyOld(key, phone, pageInfo, picKey, picCode);
		} catch (Exception e) {
			e.printStackTrace();
			pageInfo.setCode(ResultInfo.SERVER_ERROR.getCode());
			pageInfo.setMsg(ResultInfo.SERVER_ERROR.getMsg());
    ...
    public @ResponseBody Object sendModiPhone(HttpServletResponse response, HttpServletRequest request,
            String targetPhone, String picKey, String picCode) {

		PageInfo pageInfo = new PageInfo();
		try {
            String key = CommonMethodUtil.getRedisUidKeyByRequest(request);
            pageInfo = smsService.sendModiPhone(targetPhone, key, pageInfo, picKey, picCode);
		} catch (Exception e) {
			e.printStackTrace();
			pageInfo.setCode(ResultInfo.SERVER_ERROR.getCode());
			pageInfo.setMsg(ResultInfo.SERVER_ERROR.getMsg());
