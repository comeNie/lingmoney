...
<!DOCTYPE html>
<!-- saved from url=(0044)http://42.96.185.111/ibm/20141205/index.html -->
<html lang="zh-CN">
<head>
...
.box {
	width: 100%;
	background: url('/resource/images/bg.jpg') no-repeat center top;
...
.btn {
	...
	color: #fff;
	font-size: .32rem;
	background: url("/resource/images/laxinbtn.jpg");
	background-size: 100%;
}

.btn.end {
	background: url("/resource/images/laxinbtn_hui.png");
	background-size: 100%;
}
...
<script type="text/javascript">
	...
	wx.ready(function(){  
	    ...
	    wx.onMenuShareTimeline({  
	    	title: '领钱儿--邀请好友注册购买,最低可获50元话费.', // 分享标题  
	    	desc: '领钱儿 安全便捷的综合互联网金融服务平台--为您轻松解决理财问题', // 分享描述
	        link: 'http://test.lingmoney.cn/wechat/recommend.html?shareUrl=sharerecommend&referralTel=${referralTel}',  
	        imgUrl: 'http://static.lingmoney.com.cn/info/share1.jpg' // 分享图标
	    });  
	    
	    wx.onMenuShareAppMessage({
	        title: '领钱儿--邀请好友注册购买,最低可获50元话费.', // 分享标题
	        desc: '领钱儿 安全便捷的综合互联网金融服务平台--为您轻松解决理财问题', // 分享描述
	        link: 'http://test.lingmoney.cn/wechat/recommend.html?shareUrl=sharerecommend&referralTel=${referralTel}', // 分享链接
	        imgUrl: 'http://static.lingmoney.com.cn/info/share1.jpg', // 分享图标
...
<script type="text/javascript">
	...
	$(function(){
// 		var number=GetQueryString("");
// 		if(number!="" && number!=null){
// 			$(".number").html(number);
// 		}
	    
	    var test = window.location.href;
	    showPop('<p style="text-align: center;padding:.2rem 0">'+test+'</p>');
	    
		var Wheight=$(window).height();
		$(".box").css({"min-height":Wheight+'px',"overflow":"scoll"});
	   ...
	   $("#passwordre").on("blur",function(){
	    	regPassword2("#password","#passwordre")
	    }) 
	   ...
	   $(".getCode").on("click",function(){
	    	if($(this).hasClass("click")){
	    		...
	    		if(regPhone("#phone")){
	    			...
	    			$.ajax({
				    	url:"/userInterface/sendSMS.html",
				    	type:"post",
				        dataType:"json",
				        data:{
				        	telephone:$("#phone").val()
	    ...
	    $("a.btn").on("click",function(){
	    	if($(this).hasClass("end")){
	    		...
	    		if($("a.btn").hasClass("click")){
		    		return
		    	}
	    		if(regPhone("#phone") && regPassword("#password") && regPassword2("#password","#passwordre")){
	    			$("a.btn").addClass("click");
	    			$.ajax({
				    	url:"/userInterface/register.html",
				    	type:"post",
				        dataType:"json",
				        data:{
				        	account:$("#phone").val(),
				        	password:$("#password").val(),
				        	invitationCode:$(".number").text(),
				        	verificationCode:$("#code").val()
		                },
				        success:function(data){
				        	if(data.code==0){
				        		//注册成功
				        		showPop('<p style="text-align: center;padding:.2rem 0">'+"注册成功"+'</p>');
				        		window.location.href="http://static.lingmoney.com.cn/share/lingmoney/index.html?from=singlemessage&isappinstalled=0";
				        	}else{
				        		showPop('<p style="text-align: center;padding:.2rem 0">'+data.msg+'</p>');
				        		$("a.btn").removeClass("click");
				        	}
				        },
				        error:function(data){
				        	showPop('<p style="text-align: center;padding:.2rem 0">'+data.msg+'</p>');
	...
	function showPop(html){
	    ...
	    var height=$(".pop strong").outerHeight(true)+$(".pop>div:first").outerHeight(true)+$(".pop span").outerHeight(true);
	    $(".pop").css({
	        "height":height+'px',
...
<body>
	<div class="wrap">
		<div class="box">
			<div class="box-form">
				<ul>
					<li><input type="text" name="phone" id="phone"
						placeholder="请输入手机号" /></li>
					<li><input type="text" name="code" id="code"
						placeholder="请输入短信验证码" /><a href="javascript:void(0)"
						class="getCode">获取验证码</a></li>
					<li><span>推荐人号码</span><span class="number">${referralTel}</span></li>
					<li><input type="password" name="password" id="password"
						placeholder="设置登录密码" /></li>
					<li style="margin-bottom: 0"><input type="password"
						name="paswordre" id="passwordre" placeholder="确定登录密码" /></li>
				</ul>
				<P class="checkbox">
					<input type="checkbox" checked />我已阅读并同意领钱儿<a
						href="javascript:void(0)">用户注册协议</a>
				</P>
				<a href="javascript:void(0)" class="btn">立即注册</a>
			</div>
		</div>
		<div class="bgPop"></div>
		<div class="pop">
		...
		<div class="dialog">
			<strong>用户注册协议</strong>
			<div style="margin: 0 auto; width: 90%">
				<div
					style="width: 100%; height: 7rem; overflow-y: scroll; font-size: .24rem; line-height: 1.5em;">
					...
					<p class="MsoNormal" style="text-indent: 2em;">
						1.5
						本协议产生法律效力后，适用于您在本平台的全部活动。如有违反而导致任何法律后果的发生，您将以自己的名义独立承担所有相应的法律责任。<span></span>
					</p>
					<p class="MsoNormal" style="text-indent: 2em;">
					...
					<p class="MsoNormal" style="text-indent: 2em;">
						3.4
						本公司向您提供的服务的具体内容、是否收取服务费及服务费的具体标准和规则，由本公司与您另行签署的具体协议以及本平台公布的规则确定。<span></span>
					</p>
					<p class="MsoNormal" style="text-indent: 2em;">
						3.5
					...
					<p class="MsoNormal" style="text-indent: 2em;">
						5.4
						您同意因使用需要，本平台及相关服务软件、应用程序将会记录您的设备号。该记录仅用于设备识别及本平台消息推送，不会用于其它任何用途。<span></span>
					</p>
					<p class="MsoNormal" style="text-indent: 2em;">
						六、隐私权保护<span></span>
					</p>
	...
	<div class="shareTip" onclick="shareTiphide();">
		<img src="/resource/images/sharetip.png">
	</div>
</body>
</html>