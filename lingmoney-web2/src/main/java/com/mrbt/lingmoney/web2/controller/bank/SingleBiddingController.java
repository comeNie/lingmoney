...
public class SingleBiddingController extends BaseController {
	...
	public @ResponseBody Object singleBidding(HttpServletResponse response, HttpServletRequest request, Integer tId) {
		...
		try {
            ...
            if (StringUtils.isEmpty(uId)) {
				...
				pageInfo = singleBiddingService.requestSingleBidding(CLIENT_TYPE, APP_ID, tId, uId);
            }
    ...
    public void singleBiddingCallBack(HttpServletResponse response, @PathVariable String note) {
...
//            pageInfo = singleBiddingResultService.requestSingleBiddingResult(APP_ID, note);
            ...
            response.sendRedirect(HX_CALL_BACK_OPER_PAGE + status + "&oper=4");
        } catch (Exception e) {
            e.printStackTrace();
        }
	...
	public @ResponseBody Object singleBiddingVersionOne(HttpServletResponse response, HttpServletRequest request, Integer tId) {
		...
		try {
			uId = CommonMethodUtil.getUidBySession(request);
			pageInfo = singleBiddingService.requestSingleBiddingVersionOne(CLIENT_TYPE, APP_ID, tId, uId);
		} catch (Exception e) {
			e.printStackTrace();
			LOGGER.error("单笔投标失败，失败原因是：" + e.getMessage());
			pageInfo.setCode(ResultInfo.SERVER_ERROR.getCode());
