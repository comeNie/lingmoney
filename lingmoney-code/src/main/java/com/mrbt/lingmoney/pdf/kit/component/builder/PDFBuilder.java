package com.mrbt.lingmoney.pdf.kit.component.builder;
...
//        this.addWatermark(writer);
    ...
    public void addWatermark(PdfWriter writer){
        // 水印图片
        Image image;
        String templatePath=PropertiesUtil.getPropertiesByKey("PDF_IMAGE_PATH");//图片
		if(!templatePath.equals("") && templatePath!=null){
			try {
	        	image = Image.getInstance(templatePath);
	            PdfContentByte content = writer.getDirectContentUnder();
	            content.beginText();
	            // 开始写入水印
	            int pageSize=writer.getPageNumber();
	            if(image!=null){
    ...
    public void onCloseDocument(PdfWriter writer, Document document) {
        // 7.最后一步,关闭文档的时候，将模板替换成实际的 Y 值,至此，page x of y 制作完毕，完美兼容各种文档size。
        total.beginText();
