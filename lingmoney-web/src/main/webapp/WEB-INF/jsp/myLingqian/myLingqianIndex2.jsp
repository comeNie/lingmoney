		...
		<c:choose>
			<c:when test="${CURRENT_USER.certification == 0 }">
				<div class="pageTip clearfix">
					<span>根据国家监管要求，领钱儿已接入华兴银行资金存管系统。用户实名认证、绑卡、投资、充值、提现前需先开通并激活银行存管账户</span>
					<a href="/myAccount/bindBankCard">立即开通</a>
				</div>
			</c:when>
			<c:when test="${CURRENT_USER.bank == 0 }">
				<div class="pageTip clearfix">
					<span>根据国家监管要求，领钱儿已接入华兴银行资金存管系统。用户实名认证、绑卡、投资、充值、提现前需先开通并激活银行存管账户</span>
					<a href="/myAccount/bindBankCard">激活E账户</a>
				</div>
			</c:when>
			<c:otherwise>
				<c:choose>
					<c:when test="${lastTime != null }">
						<div class="pageTip clearfix">
							<input type="hidden" id="theRestOfTime" value="${lastTime }">
							<span>您还有一笔未支付产品，请尽快支付。</span> <a style="float: right;"
								href="/myFinancial/finanCialManage?status=0">立即支付</a> <span
								id="theRestOfTimeSpan" style="float: right;"></span>
						</div>
					</c:when>
				</c:choose>
			</c:otherwise>
		...
		<div class="box clearfix">
			<div class="mem_confor" style="margin-top: 0px;">
				<h3 class="clearfix">
					...
					<a href="/myFinancial/redPacket?type=0&hrpType=1"><img
									src="/resource/images/coupons.png"></a>
				</h3>
						...
						<div class="my_money">
							...
							<div class="accountBalance yhcg">
								<div class="accountBalance_title">
									<span>理财资金</span>
									<!-- <a id="yhcg" onclick="licaitab(this);" href="javascript:void(0);">银行存管</a> -->
								</div>
							    <c:choose>
							         <c:when test="${(CURRENT_USER.certification == 1 || CURRENT_USER.certification == 3) && (CURRENT_USER.bank == 1 || CURRENT_USER.bank == 3)}">
							         ...
							         <c:otherwise>
							             <!-- <div class="kt_box">
							             	<p>您还未绑定银行卡，请立即前往绑定银行卡</p>
							                 <a class="operation active" href="/myAccount/bindBankCard">去绑定银行卡</a>
							             </div> -->
							         </c:otherwise>
							    </c:choose>
							</div>
							<!-- 银行存管 -->
							<div class="accountBalance lczj" style="display:none;">
								<div class="accountBalance_title">
									<span>银行存管</span>
									<!-- <a id="lczj" onclick="licaitab(this);" href="javascript:void(0);">理财资金</a> -->
								</div>
							    <c:choose>
							         <c:when test="${(CURRENT_USER.certification == 2 || CURRENT_USER.certification == 3) && (CURRENT_USER.bank == 2 || CURRENT_USER.bank == 3)}">
							             <div class="balanceBox clearfix">
		                                    <div class="b_left">
		                                        <div class="bl_1">
		                                            <div id="blbtip_2" class="tip">账户总额=可用余额+冻结金额</div>
		                                            <p>账户总额（元）<i id="b_Tip" onmouseover="showTip(1);" onmouseout="showTip(0);" >?</i>
		                                            </p>
		                                            <div class="bl_t">${availableMoney}</div>
		                                        </div>
		                                        <div class="bl_2">
		                                            <p>冻结金额（元）<span onclick="showDj(0);">详情&gt;</span></p>
		                                    ...
		                                    <div class="b_right">
		                                        ...
		                                        <a class="operation active" href="javascript:void(0);" onclick="widthDrawOrRecharge(1)">充值</a>
		                                        <a class="operation" style="margin-bottom: 0;" href="javascript:void(0);" onclick="widthDrawOrRecharge(2)">提现</a>
		                                    </div>
		                                </div>
							         </c:when>
							         <c:otherwise>
							...
							<!--  <div class="accountBalance">
								<p>账号余额：<span>
									<c:if test="${UserAccount.balance != null }">
										<fmt:formatNumber pattern="#,##0.00#" value="${UserAccount.balance}" />
									</c:if>
								</span>元</p>
							</div>
							-->
