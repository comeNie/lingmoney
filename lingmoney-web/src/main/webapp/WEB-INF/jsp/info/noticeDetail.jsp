	...
	<div class="mainbody">
		<div class="box clearfix">
			...
			<div class="mRight">
				<div class="mRight01">
					<div class="htitle">
						资讯中心 <a href="/infoNotice/list.html">返回</a>
					</div>
					<div class="zonglan ncenter news-con">
						<h2>站内公告</h2>
						<h1 title="${infoNotice_now.title }">${infoNotice_now.title }</h1>
						<div class="clearfix" style="position:relative;z-index:1;height:50px;">
							<span class="display-time">发表时间：<fmt:formatDate
									value="${infoNotice_now.pDt }" pattern="yyyy/MM/dd HH:mm:ss" /></span>
						...
						<!-- JiaThis Button END -->
						${infoNotice_now.content }
						<div class="prenext">
							<p class="pre">
								<c:choose>
									<c:when test="${infoNotice_pre==null }">
									</c:when>
									<c:otherwise>
										上一篇：<a href="/infoNotice/details.html?id=${infoNotice_pre.id}"><span
											title="${infoNotice_pre.title }">${infoNotice_pre.title}</span></a>
									</c:otherwise>
								</c:choose>
							</p>
							<p class="next">
								<c:choose>
									<c:when test="${infoNotice_next==null }">
									</c:when>
									<c:otherwise>
										下一篇：<a href="/infoNotice/details.html?id=${infoNotice_next.id}"><span
											title="${infoNotice_next.title }">${infoNotice_next.title}</span></a>
									</c:otherwise>
								</c:choose>
							</p>
						</div>
