...
<script type="text/javascript">
	$(function() {
		...
		var noCache = Date();
		$.getJSON('/financialStory/cartoonCategoryJson.html?id=' + cid,{"noCache":noCache},
			function(json) {
				if(json != null) {
					sqlJson = json.data;
				}
								...
								(function(sqlJson) {
									...
									if (itemNum > sqlJson.length) {
										...
										for ( var i in sqlJson) {
											html += "<li class='item'><a href='/financialStory/cartoonContent.html?id="+sqlJson[i].id+"' class='a-img'><img src='"+sqlJson[i].themePic+"'></a>";
...
<body>
	...
	<div class="mainbody_member">
		<div class="box clearfix">
			...
			<div class="mRight">
				<div class="mRight01">
					<div class="htitle">漫画贴</div>
					<div class="zonglan hcenter">
						<strong class="color"
							style="padding-left: 16px; background: url('/resource/images/bg_squer.jpg') no-repeat left center; font-size: 16px;">${cartoonCategory.name }</strong>
						<!-- 瀑布流样式开始 -->
						<div class="content">
							<div class="waterfull clearfloat" id="waterfull">
								<ul>
									
									<c:forEach var="item" items="${listCartoonContent }"
										varStatus="status">
										<c:if test="${status.index<4 }">
											<li class="item"><a href="/financialStory/cartoonContent.html?id=${item.id }" class="a-img"> <img
													src="${item.themePic }" />
											</a>
